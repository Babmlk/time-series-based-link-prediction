...
public class FrameworkUtils {
	...
	private static Event getEvent(Frame previous, Frame current, int v1, int v2){
		...
		if(previous.getContent().containsVertex(v1) && previous.getContent().containsVertex(v2)){//Frame anterior contem os dois nós
			wereNeighbors = previous.getContent().isNeighbor(v1, v2);
		}
		if(current.getContent().containsVertex(v1) && current.getContent().containsVertex(v2)){//Frame atual contem os dois nós
