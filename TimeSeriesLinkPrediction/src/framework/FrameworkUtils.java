...
public class FrameworkUtils {
	...
	private static TimeSeries[] readSeries(String id, ArrayList<SimilarityMetric> metrics) throws IOException{
		TimeSeries[] timeSeries = new TimeSeries[metrics.size()];
		...
		while((line = readers.get(i).readLine()) != null){
			...
			timeSeries[i] = (new TimeSeries(values,classAttribute));
			i++;
			if(i >= readers.size()){
				i = 0;
			}
	...
	public static void savePredictedScores(String id, boolean wekaFormat, ArrayList<SimilarityMetric> metrics, ArrayList<Forecaster> forecasters) throws IOException{
		...
		TimeSeries[] timeSeries = readSeries(id, metrics);
		
		for(TimeSeries series : timeSeries){
			int classAttribute = series.getClassAttribute();
